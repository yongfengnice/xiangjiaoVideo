...
import com.android.baselibrary.multithreaddownload.dbcontrol.DataKeeper;
...
public class DownLoadManager {
    ...
    private void init(Context context) {
        ...
        userID = sharedPreferences.getString("UserID","gyq");
    ...
    public int addTask(String TaskID, String url, String fileName, String filepath) {
        Log.e("test","******addtask******url="+url+"***taskID="+TaskID);
        if(TaskID == null){
            TaskID = fileName;
        }
     ...
     * @param taskID
     */
    public void saveDownloadInfo(String taskID){
        int listSize = taskList.size();
        for (int i = 0; i < listSize; i++) {
            DownLoader deletedownloader = taskList.get(i);
            if (deletedownloader.getTaskID().equals(taskID)) {
                deletedownloader.saveDownloadInfo();
                break;
            }
        }
    }
