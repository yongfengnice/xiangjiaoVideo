...
// 如果用户没有处理则让系统默认的异常处理器来处理
    ...
    private static void initMap() {
        ...
        String info = "抱歉，程序遇到错误，即将重启";

        regexMap.put(".*NullPointerException.*", info);
        regexMap.put(".*ClassNotFoundException.*", info);
        regexMap.put(".*ArithmeticException.*", info);
        regexMap.put(".*ArrayIndexOutOfBoundsException.*", info);
        regexMap.put(".*IllegalArgumentException.*", info);
        regexMap.put(".*IllegalAccessException.*", info);
        regexMap.put(".*SecturityException.*", info);
        regexMap.put(".*NumberFormatException.*", info);
        regexMap.put(".*OutOfMemoryError.*", info);
        regexMap.put(".*StackOverflowError.*", info);
        regexMap.put(".*RuntimeException.*", info);
    }
}